...
const Popular = () => {
  ...
  const visibleItems = popularData.slice(
    visibleRange.start,
    visibleRange.end + 1
  );
  ...
  const popularItems = visibleItems.map((info) => (
    <div
      key={info.id}
      className="lg:w-[23%] md:w-[23%] w-[100%] rounded-md bg-gray-200 mb-4 border-gray-200 border"
    >
  ...
  const mobileSliderData = popularData.length > 0 && (
    <div
      key={currentIndex.id}
      className="lg:w-[23%] md:w-[23%] w-[100%] rounded-md bg-gray-200 mb-4 border-gray-200 border"
    >
  ...
  return (
    <div className="w-[90%] mx-auto">
      <h3 className="my-4 text-[24px] font-bold text-center capitalize">
        Popular demands
      </h3>
      <div className="lg:flex md:flex flex-col lg:flex-row md:flex-row relative justify-between hidden">
        ...
        <button
          onClick={handlePrev}
          className="absolute left-0 bg-black p-2 text-white text-[1.5rem] rounded-md top-32"
        >
          <FaAngleLeft />
        </button>
        <button
          onClick={handleNext}
          className="right-0 absolute bg-black p-2 text-white text-[1.5rem] rounded-md top-32"
        >
          <FaAngleRight />
        </button>
      </div>
